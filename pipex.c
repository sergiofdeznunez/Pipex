...
void	first_child(char **argv, char **cmd1, char **cmd2, int pipes[], char **envp)
{
	int	infile;
...
void	second_child(char **argv, char **cmd1, char **cmd2, int pipes[], char **envp)
{
	pid_t	pid;
	int 	dest;
...
void	waitnfree(char **cmd1, char **cmd2, pid_t pid/*, int status*/)
{
	int	status;
...
int	main(int argc, char **argv, char **envp)
{
	...
	if (argc == 5)
	{
		...
		{
			if (access(argv[1], R_OK) == 0)	
				first_child(argv, cmd1, cmd2, pipes, envp);
			else
				catch_oerror(cmd1, cmd2, -1);//¿¿catch_oerr??
		}
		else
			second_child(argv, cmd1, cmd2, pipes, envp);
		waitnfree(cmd1, cmd2, pid/*, status*/);
	}
	else
